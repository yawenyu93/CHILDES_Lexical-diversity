...
The environmental differences concern quantity and quality of the language input, but the quality is much more complicated to measure. The quantity of input can be simply measured by counting the number of words or utterances spoken around children [@hart1995], whereas how to quantify its quality remains subject to debate. One approach for estimating the quality, that has been used in many research topics in language development, is lexical diversity. However, this concept is still disputable. In particular, varied similar terms have been used interchangeably with lexical diversity in many literatures, for example, lexical richness [@laufer1995], lexical density [@ure, @halliday1989], lexical variation [@linnarud; @hyltenstam] and vocabulary richness [@wimmer]. In this research, in line with the rating scales of aforementioned language development research, lexical diversity is losely defined as the range of different words used in a text, with a greater range indicating a higher diversity [@maccarthy2010].

Measuring lexical diversity is as complex as its concept. Many lexical diversity indices have been adopted within language development discpline [@rowe2008; @huttenlocher2010] and across discplines such as linguistics [@giles], speech-language pathology [@thordardottir], behavioral research [@mccarthy2010], and journalism [@burgoon]. A major problem for measuring lexical diversity is the sensitivity to text length. For example, Type-token ratio (TTR)--unique vocabulary types divided by text length (V/N)--was once the most widely used and timehonored approach, yet inherently flawed, because it is highly affected by text length [@richards2009]. In particilar, type-token ratio decreases as the total number of words or tokens sampled increases, which is clearly depicted by Heaps' law (see [@montag2018; @heaps1978]). 

Several suggestions for improving this fault have been given relating to transformations of the TTR were developed, e.g. Root TTR (T/√N) by Guiraud (1954, 1960), corrected TTR (T/ √2N) by Carroll (1964), Log TTR (logT/logN) by Herdan (1960), MATTR (moving-average of TTR) by Covington [@mattr1993; @covington]. Most variations have been widely tested and failed to project adequately lexical diversity trend of a text [@mccarthy2007; @marvern2004]. MATTR is presently the most satificatory transformation of the TTR and it has been implemented in the software WordSmith 4.0 (Oxford University Press) as the standardized type-token ratio. Initially, a window length (say 100 words) is selected. Then we compute the TTR for words 1-100, then for words 2-101, then 3-102, and so on to the end of the text. The mean of all the TTRs is computed as an indicator of lexical diversity of the entire text. Instead of one single sample of words as measured by TTR, MATTR is based on consecutive word chuncks of a text [@covington].

Other alternatives to TTR and its tranformations include vocd-D and MTLD, both of which have draw increasing attention. The vocd-D has been included in the widely used Computerised Language Analysis (CLAN) suite of programs (available on the CHILDES website at http://childes.psy.cmu.edu) and steadily became the lexical diversity index of choice for many researchers [@rowe2008; @silverman2002; @mckee2000]. The vocd-D is calculated by taking a set of random samples without replacement, in an increasing sample sizes. It begins with 100 samples of 35 tokens drawn from the text and calculates a mean TTR for all the samples. The procedure is repeated for samples of 36 tokens, 37 tokens and all the way to 100 samples of 50 tokens. All of mean TTRs are plotted in order to produce a value that is fit into a segment of the curve [@marvern2004; @mccarthy2007].

The last lexical diversity measure used in this study is MTLD. Theoretically, MTLD estimates average number of consecutive tokens for which a certain TTR is maintained (e.g. 0.72 by default). For any given sample, each token is evaluated sequentially for its TTR. For example, "I"(TTR = 1) "had"(TTR = 1) "chicken"(TTR = 1) "and" (TTR = 1) "I" (TTR = 0.8) "also" (TTR = 0.83) "had" (TTR = 0.71) and so forth. When the default TTR score is reached (here, 0.72), the factor count increases by a value of 1 and the TTR evaluations are reset. This process is repeated until the last token of the sample is evaluated for its TTR. Then the total number of tokens is divided by the total factor count. Subsequently, the same process is repeated on the reversed language sample. The final MTLD value is the mean of forward and reversed MTLD scores.
...
Our goal in this study was to explore how to appropriately measure language diversity and to investigate how parents' and children's languages are related. These questions are closely interwoven with each other. On the one hand, any measure of language diversity is always contigent on the context in which parents and children jointly participant in and reciprocally communicate. On the other, the parent-child correlation may vary with different language measures. We bootstrapped our way in answering these questions by looking at the aforementioned different measures in their parent-child correlation and how these measures relate to external measures. Our hypothesis was individual parents' and children's lexical diversity are related, in particular, the rate of language acquisition and their language skills or language proficiency. These correlations measured by lexical diversity indices could be confirmed with external measures. 
...
From the full set of `r demos %>% pull(id) %>% n_distinct()` children, we excluded those who did not meet a set of criteria. First, we excluded `r demos %>% filter(is.na(lesion)) %>% pull(id) %>% n_distinct` atypically developing children. We then excluded `r demos %>% filter(is.na(lesion), completed) %>% group_by(id) %>% summarise(n= n()) %>% filter(n < 5) %>% pull(id) %>% n_distinct` children for whom we did not have at least 5 home visits in which they spoke at least 100 words [@mccarthy2007; @marvern2004] per visit in order to accurate estimate individual vocabulary growth. 
...
After constructing individual growth trajectories, we turn to three fundamental questions in order to address the primary concern of this paper. The first question is whether the overall trajectories of children and caregivers language richness change over time. The second question is whether there are significant individual differences among participants in LDP corpus. We used mixed-effects models to investigate variations in emphasized growth curve parameters with respect to different lexical diversity indices (e.g. MTLD, TTR, vocd-D and MATTR). Therefore, we tracked not only the overall characteristics of participants’ vocabulary development, but also the nature of individual differences in their pattern of language use.
...
ldp_tidy <- ldp_all%>%
  ...
  mutate(type = if_else(measure %in% c("cdi", "sentence", "ppvt"),
                        "external", "corpus")) %>%
  group_by(measure) %>%
...
ldp_tidy %>%
  ungroup() %>%
  mutate(measure = factor(measure, levels = c("cdi", "sentence", "ppvt", 
...
```{r}
ldp_tidy %>%
  filter(measure == "mtld", person == "child") %>%
  group_by(age) %>%
...
Children vary widely in their intercept and slope of vocabulary growth trajectory. We first evaluate predictors of child's growth parameters generated by MTLD, MATTR, vocd-D and TTR. A growing body of previous work demonstrates significant influence of caregiver's speech on child's language development [e.g., @huttenlocher1991; @hurtado2008; @rowe2008; @rowe2015; @huttenlocher2010]. In LDP corpus, caregivers' intercept does not relate to their child's intercept, as shown in Table 2. Because the initial language aptitude is represented by one of growth parameters-intercept-we seprate confounding contextual relation from caregiver-child conversation sample. Yet, caregiver's slope relates to child's growth rate of vocabulary diversity. Table 2 demonstrates a positive relation between caregiver's slope and child's slope that are deriving from MTLD and MLU, while MATTR generates a moderately negative relation between them. This finding aligns with previous work in which mothers fine-tune language usage, specifically vocabulary choice, across development in connect to their children's level of understanding and language skills [@dan2016].
...
ldp_intercept %>%
        ...
        filter(type == "slope")%>%
        group_by(measure)%>%
        ggplot(., aes(x = k, y = m, color = measure))+
        geom_jitter(alpha = .05) +
        geom_smooth(method = 'lm', se = F, size = 1, formula = y~x)+
        facet_wrap(~measure, scales = "free")+
        labs(x="child's slope", y ="mother's slope")+
        theme_classic()+
        scale_color_ptol() +
        scale_fill_ptol() 
```

A growing body of research documents a relation between socioeconomic status and children's vocabulary development [@hart2995; @lawrence1996; @hoff-ginsberg1982; @hoff2003]. Our results also show household income is a significant predictor of children's lexicon diversity. Table 3 presents a significant correlation between household income and child's slope generated by MTLD, PPVT and MLU. Surprisingly, there is no correlation between household income and mother's slope of any vocabulary measure, but its significant relation to how mother change the amount of child-directed speech across age is documented. In addition, income also relates to the amount of talk children produced across development, suggesting its role on child's language development is partially medited by the quantity of child-directed speech.
...
ses_slope <- ldp_tidy %>%
        ...
        group_by(subject)%>%
        mutate(income = floor(mean(income, na.rm = T)))%>%
        unique()%>%
        select(-intercept)%>%
        spread(type, value) %>%
        group_by(measure) %>%
        summarise(cor = cor(slope, income))%>%
        rename(., slope = cor)


apa_table(
        ses_slope,
        caption = "Correlation between Family Income and Child's Lexical Diversity Growth Rate"
)
...
## Sequential analysis
When looking into existing lexical diversity indices, nonsequential analysis is still a common approach. One reason of its being ubiquitous relates to the advantage of avoiding local clustering. However, it may lead to a distorted way of overall text (Malvern et al. 2004). MTLD is an exception. Theoretically, MTLD estimates average number of consecutive tokens for which a certain TTR is maintained (e.g. 0.72 by default). For any given sample, each token is evaluated sequentially for its TTR. For example, "I"(TTR = 1) "had"(TTR = 1) "chicken"(TTR = 1) "and" (TTR = 1) "I" (TTR = 0.8) "also" (TTR = 0.83) "had" (TTR = 0.71) and so forth. When the default TTR score is reached (here, 0.72), the factor count increases by a value of 1 and the TTR evaluations are reset. This process is repeated until the last token of the sample is evaluated for its TTR. Then the total number of tokens is divided by the total factor count. Subsequently, the same process is repeated on the reversed language sample. The final MTLD value is the mean of forward and reversed MTLD scores.
...
# Discussion
Previous research has shown large individual difference in children's language skills and the rate of their language growth [@huttenlocher1991; @fenson1994; @hart1995] relating to quantity and quality of language input [@hoff2003; @huttenlocher1991], that vary in SES [@hess1965; @Heath1983; @hoff-ginsberg1982; @hart1995; @rowe2008]. This study also documented the large variation in children's vocabulary diversity and its relation with maternal language and SES (i.e. family income). However, until our study, little evidence has been presented regarding how this variation and relation differ with respect to various measures. Different language measures have generated different results relating to the variation in individual vocabulary diversity and the rate of their lexcial diversity growth as well as their relation to maternal language. 

The results regarding the use of vocd-D on capturing child's language growth across development contradict to the findings reported in [@duran]. One explanation is the difference in the way the texts of child's utterance are cleaned. Duran et al (2004) was not able to implement the vocd-D measure on the most representative sample of child's speech as limited by the exsisting techniques of text mining. We, now, are able to lower the case of all words, remove all the untelligible utterances and special identifies that may be mistakenly counted as novel word type by the programme, so that accurate the measurement results.
