...
utter_kid_us <- utter_us %>%
  ...
  mutate(stem = tolower(stem),
         stem = removeWords(stem, murmur),
...
p_us <- utter_p_us %>%
  ...
  left_join(select(kid_us, corpus, subject, session, age))%>%
  ungroup(.)%>%
  group_by(corpus, age, subject)%>%
  summarise(utts = paste0(utts, collapse = " ")) %>%
  filter(!subject == "NA")

us_id <- unique(p_us$subject)

# tokenize utterances
kid_tokenized_us <- kid_collapsed_us %>%
  filter(subject %in% us_id) %>%
  ungroup(.) %>%
...
# filter child with more than 3 observations
...
p_uk <- utter_p_uk %>%
  ...
  select(-target_child_age) %>%
  rename(corpus = corpus_id, session = transcript_id) %>%
  left_join(select(kid_uk, corpus, subject, session, age))%>%
  ungroup(.)%>%
  group_by(corpus, age, subject)%>%
  summarise(utts = paste0(utts, collapse = " ")) %>%
  filter(!subject == "NA")

uk_id <- unique(p_uk$subject)

# tokenize utterances
kid_collapsed_uk <- kid_uk %>%
  filter(subject %in% uk_id) 
...
mom_stat <- speaker_stat %>%
  ...
  mutate(mom_num_utter = sum(num_utterances, na.rm = TRUE),
         mom_num_type = sum(num_morphemes, na.rm = T),
         mom_mlu = mom_num_type / mom_num_utter)%>%
...
kid_stat <- speaker_stat %>%
  ...
  mutate(kid_num_utter = sum(num_utterances, na.rm = TRUE),
         kid_num_type = sum(num_morphemes, na.rm = T),
         kid_mlu = kid_num_type / kid_num_utter)%>%
