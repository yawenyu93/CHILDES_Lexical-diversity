...
recode_combine <- recode_data %>%
  ...
  group_by(subject)  %>%
  mutate(mom_lag = lag(mtld.x, n=4),
         kid_lag = lag(mtld.y, n=4)) %>%
...
ggplot()+
  geom_smooth(aes(x=age, y=mtld.x, colour = "mother"), data = recode_combine)+
  geom_smooth(aes(x=age, y=mtld.y, colour = "children"), data = recode_combine)+
...
anova(mean_model, slope_model) # kid's slope
...
kid_effects <-  kid_effects %>%
  mutate(subject = rownames(kid_effects),
         age = `(Intercept)` + fixed_effects[1]) %>%
  select(-one_of(drop))

colnames(kid_effects)[[2]] <- paste(names(kid_effects)[[2]], c(time_table[[i]]),sep = "")

recode_age_combine <- merge(x = recode_age_combine, y = kid_effects, by = c("subject"))} 
...
ggplot() + 
  geom_errorbar(data=cor_mean_data, mapping=aes(x=age, ymin=lower, ymax=upper), width=1, size=2, color="navy")+
...
ggplot() + 
  geom_errorbar(data=cor_slope_data, mapping=aes(x=age, ymin=lower, ymax=upper), width=1, size=2, color="navy")+ 
...
#### increases steadily, so does variation
```
