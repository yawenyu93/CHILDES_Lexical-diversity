...
kid_mom_model <- lmer(kid_mtld ~ 1 + log(age) + mom_mtld + (1+log(age)|subject), data = complete_data)
anova(age_mom_model, kid_mom_model) 

# summary optimal model
Model1 <- kid_mom_model
summary(Model1)
...
kid_temp <- kid_effects %>%
  ...
  mutate(kid_intercept = kid_intercept + kid_effects$random) %>%
  select(-c(age, random, mom_mtld, kid_slope, mom_slope)) 
...
kid_data <- kid_effects %>%
  ...
  mutate(kid_intercept = kid_intercept + kid_effects$random) %>%
  left_join(y = subjs, by = c("subject"))  %>%
  select(-c(age, lesion, note, mom_mtld, mom_slope, random)) %>%
...
Model2 <- lm(kid_intercept ~ mom_slope, data = kid_data)
summary(Model2)
...
Model3 <- lm(kid_slope ~ mom_mean + kid_intercept, data = kid_data)
summary(Model3)
```
...
```{r}
###### Final model: 
###### kid_mtld = -42.5 + (1.69 * mom_slope - 19.5) +  0.173 * mom_mtld + (14.2 + 0.014 * mom_mean - 0.186 * kid_intercept) * log(age)
...
# predict kid's intercept at 15 month
  kid_mom_model <- lmer(kid_mtld ~ 1 + log(age) + mom_mtld + (1+log(age)|subject), data = df)
  kid_temp_data <- as_data_frame(ranef(kid_mom_model)$subject) 
  kid_temp_data <-  kid_temp_data %>%
    mutate(subject = as.integer(rownames(kid_temp_data)),
          random = `(Intercept)`) %>%
    select(subject, random)
  
  mom_mean_15 <- df %>%
    filter(age == 15) %>%
    summarise(mean = mean(mom_mtld)) %>%
    mutate(mom_slope = mean(mom_data$mom_slope))
  ...
  data <- kid_temp_data %>%
    mutate(age = 15,
         ...
         mom_slope = mom_mean_15$mom_slope) %>%
   mutate(kid_intercept = predict(Model1, newdata = .)) %>%
   mutate(kid_intercept = kid_intercept + kid_temp_data$random) %>%
   select(-c(age, mom_mtld, mom_slope, random)) %>%
...
# predict every child's growth trajectory  
  ...
  left_join(y=data, by=c("subject")) %>%
  mutate(prediction = (-42.5 + 1.69 * mom_slope - 19.5 +  0.173 * mom_mtld + (14.2 + 0.014 * mom_mean - 0.186 * kid_intercept) * log(age)))  
...
temp %>%
  make_curve(.) %>%
  filter(subject %in% head(unique(.$subject), 15)) %>%
  ggplot() +
